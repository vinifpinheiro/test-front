...
const MainData = () => {
  ...
  useEffect(() => {
    httpDefault
      .get<IDataResults[]>(`/api`)
      .then((response: { data: SetStateAction<IDataResults[]> }) => {
      ...
      <main>
        {nameData.map((nameData) => (
          <div key={nameData.id} className={styles.data__bank}>
            <h1 className={styles.h1__username}>
              <FaUser /> {nameData.name}
            </h1>
            <h1 className={styles.h1__userDocument}>
              Documento: {nameData.document}
            </h1>
            <div className={styles.bank__specification}>
              ...
              <h1 className={styles.h1__date}>Data do relatório: {date}</h1>
            </div>
          </div>
        ))}
      </main>
